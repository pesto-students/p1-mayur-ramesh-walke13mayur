...
function isVovel(str){
    const vovelMap = new Map();
    for(const char of str){
        const vovels = 'aeiou'.includes(char.toLowerCase());
        if(vovels){
            const vovelsCount = vovelMap.get(char) || 0 ;
            vovelMap.set(char.vovelsCount +1);
        }
    }
    return vovelMap;
}
console.log(isVovel("mayur"));